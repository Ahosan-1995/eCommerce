...
export async function POST (request){
    ...
    const {date,time,seats,bookBy,email,phone}=await request.json();
    await conncetMongoDB();
    await menus.create({category,name,description,ingredient,price,image});
    await booking.create({date,time,seats,bookBy,email,phone});
...
// since we get data by menu so we will get it my menu inside our componnet
export async function GET (){
    await conncetMongoDB();
    const menu = await menus.find();
    return NextResponse.json({menu});
    const bookings = await menus.find();
    return NextResponse.json({menu});
}
