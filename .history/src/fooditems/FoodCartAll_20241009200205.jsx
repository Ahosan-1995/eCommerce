...
const FoodCartAll = async () => {
  ...
  if (Array.isArray(menu)) {
    ...
    if (bangladeshiFoodItems.length > 0  && content.length < 3) {
      content.push(
    ...
    if (dessertItems.length > 0  && content.length < 3) {
      content.push(
        <FoodCartAllExperimental
          key="Dessert"
          menu={dessertItems}
    ...
    if (SoftDrinks.length > 0  && content.length < 3) {
      content.push(
        <FoodCartAllExperimental
          key="SoftDrinks"
          menu={SoftDrinks}
    ...
    if (ThaiFood.length > 0  && ThaiFood.length < 3) {
      content.push(
        <FoodCartAllExperimental
          key="ThaiFood"
          menu={ThaiFood}
    ...
    if (FastFood.length > 0  && content.length < 3) {
      content.push(
        <FoodCartAllExperimental
          key="FastFood"
          menu={FastFood}
